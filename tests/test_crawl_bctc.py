...
class CrawlBCTC(BasePlaywrightCrawler):
    ...
    "IV. Các công cụ tài chính phái sinh và các khoản nợ tài chính khác": "derivative_financial_instruments_and_other_financial_liabilities",
    "V. Vốn tài trợ, uỷ thác đầu tư mà ngân hàng chịu rủi ro": "grants_trusted_funds_and_borrowings_with_risks_borne_by_the_bank",
    ...
    async def _upsert_to_postgresql(self, results: list[dict[str, Any]]) -> None:
        try:
            async for db in async_session_scope():
                model_path = "src.shared.infrastructure.db.models.BalanceSheetTypeFour"
